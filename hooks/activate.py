...
from vm import NIVUUS_PACKAGES_DIR_DEFAUT, poser_projfs, poser_vb_audio

RACINE = pathlib.Path(__file__).resolve().parents[1]

# --- Tâche 7 : où `console` va chercher `agent.exe`, jamais deviné --------
# 🔴 Pas `<drivers_dir>/agent/agent.exe` (dérivé de `guest_workdir`, réponse
# DE `console` que ce hook ne reçoit jamais — `activate_cli.py:107-108`
# n'indexe que les réponses du package appelant ; `/etc/nivuus/packages.json`
# est d'ailleurs absent sur cette machine). Le chemin RÉEL, fixe, que
# `console` lit lui-même : `console/guest/fetch_payload.py:61` —
...
# garantit que les RECONSTRUCTIONS futures. Voir le rapport, § Réserves.
