...
- **La compilation croisée du crate `agent` NE FONCTIONNE PAS** depuis l'hôte
  Linux, et ce n'est pas un réglage à trouver : `aws-lc-sys` (via str0m) et
  `audiopus_sys` compilent du C pour la cible Windows, ce qui exige un
  cross-compilateur MSVC absent de l'hôte. L'échec est antérieur au chantier A
  et indépendant de lui (vérifié : il se reproduit sans les modifications
  d'Opus). **La seule vérification du code Windows est le build réel sur la
  VM**, par `scripts/build-agent.sh`. Ne pas perdre de temps à faire marcher
  `cargo check --target`.
- **Ce que la cible Windows permet malgré tout** : vérifier un morceau de code
  ISOLÉ dans un crate jetable ne dépendant que de `windows` et `anyhow` — c'est
  ainsi que le code WASAPI de la tâche 5 a été validé avant d'entrer dans ce
  plan. Utile en dépannage, jamais nécessaire au déroulé normal.
...
- [ ] **Étape 6 : Vérifier la suite complète, puis le build sur la VM**
...
- [ ] **Étape 3 : Vérifier la suite Linux, puis le build sur la VM**
...
Attendu : tous les tests existants verts sous Linux (le module est exclu par
`#![cfg(windows)]`), puis `Finished` au build sur la VM.
...
sleep 3
# Son joué par l'API multimédia, dans la session interactive.
#
# NE PAS utiliser `[Console]::Beep` : il pilote le haut-parleur PC par une
# API matérielle qui ne traverse PAS le mixeur WASAPI. La sonde rend alors
# `crete = 0` — un faux négatif parfaitement convaincant, qu'on attribuerait
# à tort à un périphérique détourné. Constaté à l'exécution du 28/07.
node scripts/winrm.js "1..5 | ForEach-Object { (New-Object Media.SoundPlayer 'C:\\Windows\\Media\\Windows Ding.wav').PlaySync(); Start-Sleep -Milliseconds 300 }"
sleep 15
node scripts/winrm.js "Get-Content C:\dev\agent.log -Tail 20 | Out-String"
```
...
Attendu : tests Linux inchangés et verts, puis `Finished` au build sur la VM.
...
- [ ] **Étape 9 : Compiler, tester, puis vérifier le build sur la VM**
...
Attendu : tous les tests verts, puis `Finished` au build sur la VM.
...
- [ ] **Étape 6 bis : Activer le codec Opus sur le constructeur `Rtc`**

**Sans cette étape, rien de ce qui précède ne sert à rien** : `Rtc::builder()`
appelle `clear_codecs()` puis n'active explicitement que H.264, donc aucun type
de charge utile Opus n'est proposé dans la réponse SDP et la piste audio ne se
négocie **jamais**. `Event::MediaAdded` ne rendrait alors aucun `mid` audio, et
la branche a3 resterait morte en silence.

Le PT 111 présent à `agent/src/transport.rs:1083` ne dispense pas de cette
ligne : cette table sert à *retrouver* un type de charge utile déjà négocié,
elle n'en déclare aucun.

Dans `Session::new`, ajouter `.enable_opus(true)` à côté de
`.enable_h264(true)` :
...
```rust
        let mut rtc = Rtc::builder()
            .clear_codecs()
            .enable_h264(true)
            // Sans cette ligne, aucun type de charge utile Opus n'est proposé
            // et la piste audio ne se négocie jamais — la branche a3 reste
            // morte sans le moindre message.
            .enable_opus(true)
```

Faire de même sur le `Rtc::builder()` du pair, dans le test de bouclage étendu à
l'étape 8 : un pair qui n'active pas Opus ne négociera pas la piste, et le test
...
- [ ] **Étape 10 : Compiler, tester, puis vérifier le build sur la VM**
...
- [ ] **Étape 2 bis : Étendre le harnais de vérification à l'audio**

**Fichier** : `client/verify-webrtc.mjs` (modifier)

Ce harnais existe déjà : il pilote Chrome sans interface par le protocole
DevTools et lit les vraies statistiques `getStats()` — celles que le navigateur
calcule, pas une auto-évaluation du client. Il prouve aujourd'hui que
`framesDecoded` et `framesReceived` **augmentent**, et que les dimensions sont
plausibles.

L'étendre à l'audio rend objectif ce que la recette établirait autrement par
« j'entends quelque chose ». Trois ajouts :

1. **Relever la piste audio.** La fonction qui extrait les compteurs
   `inbound-rtp` filtre sur `stats.kind === 'video'` ; il en faut le pendant
   pour `'audio'`, rendant `bytesReceived`, `packetsReceived`, `packetsLost`,
   `jitter` et `estimatedPlayoutTimestamp`.
2. **Affirmer que l'audio traverse** : `bytesReceived` et `packetsReceived`
   doivent **augmenter** entre les deux relevés, exactement comme pour la
   vidéo. Un compteur non nul mais figé prouverait seulement qu'un paquet est
   passé une fois.
3. **Mesurer le décalage A/V sans oreille humaine.** Chrome expose
   `estimatedPlayoutTimestamp` sur chaque `inbound-rtp` : c'est l'instant, sur
   une horloge commune, auquel le navigateur estime restituer l'échantillon
   courant de cette piste. **La différence entre celui de l'audio et celui de
   la vidéo est le décalage A/V tel que le récepteur le voit**, en
   millisecondes. C'est exactement ce que la correction du `wallclock` de la
   tâche 7 était censée rendre juste.

   > **Périmé — établi faux par le livrable de la même branche.** Ce plan
   > affirmait encore que Chrome expose ce champ ; `2026-07-28-audio-resultats.md`
   > (§ décalage A/V) établit le contraire **deux fois**, à `Object.keys()` sur
   > les deux pistes (vidéo et audio), à deux relevés espacés de 6 s : la clé
   > `estimatedPlayoutTimestamp` est structurellement absente de l'entrée
   > `inbound-rtp` sur Chrome 150.0.7871.181 / Debian 13 headless — pas
   > seulement `null`/`undefined`, absente de l'objet. Le harnais
   > (`client/verify-webrtc.mjs`) reste écrit conformément à ce brief (il
   > fonctionnera si le champ apparaît sur un autre Chrome/plateforme), mais
   > distingue explicitement « pas de piste audio » de « champ absent de
   > `getStats()` » plutôt que de rendre un message unique trompeur. Le
   > décalage A/V rapporté au §12 de la spec (audio-design, «≈ −10 ms ») a été
   > mesuré à l'oreille (relevé humain, voir `audio-resultats.md` §3), pas par
   > ce harnais.

Rendre le décalage dans la sortie, et le comparer aux seuils de gêne de
l'ITU-R BT.1359 : **45 ms si l'audio devance, 125 ms s'il retarde**. Le signe
compte — un audio en avance gêne près de trois fois plus tôt qu'un audio en
retard.

**Ne durcis pas le code de sortie sur le décalage A/V** : c'est une mesure, pas
encore un critère éprouvé. Affiche-la, ne fais échouer que sur l'absence
d'audio.

Vérifier que le harnais reste utilisable **sans** piste audio : une session
vidéo seule ne doit pas échouer parce que l'audio manque. Le harnais sert aussi
à la recette vidéo.
...
3. **Preuve automatique d'abord** : lancer le harnais étendu pendant qu'un son
   joue sur la VM, et relever sa sortie. C'est elle qui établit objectivement
   que l'audio traverse la chaîne — débit, paquets reçus, pertes, gigue,
   décalage A/V. Un son connu se joue par
   `(New-Object Media.SoundPlayer 'C:\\Windows\\Media\\Windows Ding.wav').PlaySync()`
   — **jamais** `[Console]::Beep`, qui ne traverse pas le mixeur WASAPI.

   Puis, **et seulement pour ce que la mesure ne couvre pas** : un humain ouvre
   le client et confirme que le son est effectivement audible et intelligible.
   La mesure prouve que des paquets Opus arrivent et sont décodés ; elle ne
   prouve pas qu'ils portent le bon signal plutôt que du bruit.
