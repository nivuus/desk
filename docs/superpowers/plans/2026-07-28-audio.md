...
4. Dans `write_frame`, remplacer l'appel à `writer.write` :
...
```rust
        ...
        let capture_a = instant_from_pts(self.clock_origin, unit.pts_90k, CLOCK_RATE_HZ as u32);
...
```rust
    #[test]
    fn l_instant_de_capture_se_reconstruit_depuis_l_origine_et_l_horodatage() {
        // Non-régression sur la correction de la synchro A/V : `write_frame`
        // doit annoncer l'instant de CAPTURE, pas celui de l'écriture. Ce
        // test verrouille la conversion sur laquelle il s'appuie ; l'usage
        // effectif est vérifié par la recette (décalage A/V mesuré).
        use crate::clock::instant_from_pts;
        use crate::h264::CLOCK_RATE_HZ;

        let origine = Instant::now();
        // Une image capturée à 2 s de l'origine porte le PTS 180 000.
        let capture = instant_from_pts(&origine, 180_000, CLOCK_RATE_HZ as u32);
        assert_eq!(capture, origine + Duration::from_secs(2));
        assert!(
            capture < Instant::now() + Duration::from_secs(3),
            "l'instant reconstruit doit rester ancré sur l'origine, pas sur l'horloge courante"
        );
    }
```

**Note :** `instant_from_pts` prend `origin` par valeur (`Instant` est `Copy`) —
écrire `instant_from_pts(origine, ...)` sans `&`. Corriger l'appel ci-dessus si
le compilateur le signale.
...
couvrir les deux médias. Remplacer la condition sur
`self.video_write_pending_drain` par une condition qui vaut si **l'un ou
l'autre** drapeau est posé, et remettre les deux à `false` en même temps — un
seul `handle_input(Input::Timeout(..))` draine la file de charge des deux
pistes.
...
Dans `client/src/stats.ts`, dans la boucle `report.forEach`, ajouter la
collecte audio à côté de la vidéo :

```typescript
        let inboundAudio: RTCInboundRtpStreamStats | undefined;
```

(déclarée avec `inbound` et `pair`), puis dans le `forEach` :
...
Et dans la composition de `element.textContent`, ajouter en fin de tableau :
...
avec, à côté du calcul de `mbps` et sur le même modèle (différence de
`bytesReceived` sur la différence d'horodatage), le calcul de `audioKbps` à
partir d'un instantané audio précédent. Déclarer l'instantané audio comme la
même `interface Snapshot`, sans `framesDecoded` — ajouter à l'interface un
champ optionnel plutôt que d'en créer une seconde :
...
reste valable : pour l'audio, `framesDecoded` vaut 0 et n'est pas lu.

**Sans ces chiffres, la recette n'a rien à lire** et « ça a l'air de marcher »
tiendrait lieu de résultat.
