...
try {
    ...
    for (let i = 0; i < 90 && !session; i += 1) {
        await dormir(1000);
        for (const c of await cibles()) {
            if (c.type === 'page' && /index\.html\?session=/.test(c.url)) {
    ...
    if (!session) throw new Error("aucune page de session : la shell a-t-elle recu une fenetre ?");
