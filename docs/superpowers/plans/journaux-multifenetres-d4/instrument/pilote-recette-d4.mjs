#!/usr/bin/env node
// Recette D4 — tâche 9 : les trois critères de réception de la capture
// mutualisée, en conditions de produit.
//
...
//
//   PHASE=montee   critère 1 — ouvrir des Bloc-notes un par un jusqu'au refus,
//                  relever combien DIFFUSENT réellement (images décodées côté
//                  navigateur, pas « une page s'est ouverte »).
...
async function main() {
    ...
    try {
        ...
        vmItFichier('preparerd4', 'preparer-d4.ps1');
        await dodo(6000);
        ...
        const ouvrirFenetre = async (n, attente = 40) => {
            ...
            vmIt(`ouvrird4-${n}`, `Start-Process notepad -ArgumentList 'C:\\dev\\fenetre-${n}.txt'\nStart-Sleep -Seconds 2`);
