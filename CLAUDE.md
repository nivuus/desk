...
- **lot 33 — le recadrage et la fenêtre suivent le viewport (31 août 2026)** — [résultats](docs/superpowers/plans/2026-08-31-lot33-viewport-suivi-resultats.md) — 🔴 **le `Resize` du navigateur était JETÉ, et c'est mesuré sur l'agent de production** : 34 demandes, rapports d'aspect de **1,105 à 3,559** (dont un `5118x1438`), toutes servies à **1428×1080 = 1,3222** — le `#remote` étant `object-fit: contain` sur `--video-letterbox: #000`, l'écart **se peint littéralement en noir**. 🔴 **Les deux moitiés du défaut se couvraient l'une l'autre** : `viewport_recu` ignorait tout viewport sur une session `Vivante`, et le client n'en réannonçait aucun. Remède : **une règle pure (`taille_pour_viewport`), deux processus, UNE seule mesure côté client** — le capteur retaille fenêtre + recadrage + encodeur **sans relâcher la duplication** (C1 de D1), le superviseur corrige `taille_sortie` et repose ; même `HMONITOR`, même borne, donc le second geste est un `no-op`, et une divergence se résout en une seconde. 🔵 **La borne est désormais la ZONE DE TRAVAIL** — premier lecteur de `rcWork` du dépôt — ce qui sort les 48 rangées de la barre des tâches secondaire du cadre, rend à l'application ses 48 px de contenu, et corrige le clic dans les 4,4 % bas. ⚠️ **AUCUN mode d'affichage n'est changé : ce n'est PAS le chemin que D9 a retiré.** 🔴 **CE QUE CE LOT N'ÉTABLIT PAS : rien n'est déployé, aucune session ne l'a exercé** (rôle `client` exclusif, propriétaire connecté), **personne n'a regardé une image**, et **un viewport plus large que la borne garde ses bandes noires** — un test le fige. ⚠️ Une hypothèse à moi — « le placement périodique est en bataille permanente » — a été **RÉFUTÉE** par la mesure restreinte au jour même (6 replacements, pas 30 430).
...
- 🔴 **GRANDIR AU-DELÀ DE LA SORTIE RESTE IMPOSSIBLE** (lot 33). Le recadrage
  suit désormais le viewport, mais `taille_retenue` borne par un `min` axe par
  axe : un viewport plus large que la sortie garde ses bandes noires (mesuré :
  `5118x1438` demandé, `1428x538` servi). **La seule voie connue est de créer
  la sortie à `TAILLE_MAX_SORTIE`**, et elle est **écartée par ce lot avec ses
  raisons** : D8 a établi qu'une sortie ne naît PAS à la taille demandée, la
  relation entre taille demandée, `DesktopCoordinates` (1428) et
  `DXGI_OUTDUPL_DESC` (1860) n'est **pas comprise**, et `TAILLE_MAX_SORTIE`
  n'est pas calibrée. **Décision du propriétaire.**
