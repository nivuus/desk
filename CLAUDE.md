...
- **lot 3 — la campagne sur la VM, résultats PARTIELS : six items sur douze chiffrés (5 septembre 2026)** — [résultats partiels](docs/superpowers/plans/2026-09-05-lot3-campagne-vm-resultats-partiels.md) — la suspension du lot 3 est levée par un **harnais re-situé sur l'appliance**, posé à côté de `harnais.sh` qui reste intact. Chiffres datés : **latence** (item 7) — `captureTime` absent de 9 084/9 084 trames, substitut 9,18/9,04 ms au nominal contre 42,27/39,72 ms sous `netem adsl` ; **extinction** (item 5) — **3 sorties virtuelles orphelines** pour 3 fenêtres, deux exécutions, contre **0** au témoin à froid ; **file du capteur** (item 6) — `PROFONDEUR_MAX=64`, **0 refus pour 457 attachements** ; **replis micro** (item 9) — `Introuvable` couru **455** fois ; **applications** (item 11) — **41 au catalogue, 34 `non-mesuree`** ; **`Resize`** (item 12) — trois viewports servis exactement, **aucun maillon fautif**. 🔴 **LE CONSTAT LE PLUS TRANSFÉRABLE : TROIS CONTRÔLES QUE LE PLAN PRESCRIVAIT NE POUVAIENT PAS ÉCHOUER** — le `grep "NonMesuree"` (variant Rust jamais journalisé), le TÉMOIN `attache au capteur` (le produit écrit `attaché`, avec l'accent), et le commentaire de sonde dictant que le sender report RTCP alimente `captureTime` (il alimente `remoteTimestamp`). ⚠️ **CE DOCUMENT NE CLÔT PAS LE LOT** : les items 1, 2, 3, 4, 8 et 10 sont **entiers et non joués**, aucune ligne de produit n'a été modifiée, personne n'a regardé une image ni écouté un son.
