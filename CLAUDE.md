...
>    voir [les résultats partiels](docs/superpowers/plans/2026-09-05-lot3-campagne-vm-resultats-partiels.md).
...
- **navigation-hub-unique — le hub devient la SEULE surface (31 août 2026)** — [résultats](docs/superpowers/plans/2026-08-31-navigation-hub-unique-resultats.md) — le hub tient la session de contrôle, montre ses fenêtres et son pont fichiers ; un onglet est élu **porteur** par Web Locks (`bureau/porteur.ts`) et les autres suivent **sans jamais voir d'erreur** ; `shell.html` devient une **redirection permanente** (les PWA installées portent `shell.html?app=` dans un manifeste `blob:` qu'elles ne reliront jamais, legs G5) ; `start_url` migre vers la racine pendant que **`id` reste figé** ; le jeton est vérifié **frais avant chaque usage** (`assurerAccesFrais`), fermant le défaut où `rafraichirSiNecessaire` n'avait **aucun appelant de production**. 🔴 **Le constat le plus utile est que les QUATRE défauts trouvés par les revues venaient TOUS DU PLAN, pas des implémenteurs** — la boucle de revue a servi *contre* le plan. ⚠️ **CE QUE CE LOT N'ÉTABLIT PAS, et ce sont les deux limites qui comptent : AUCUNE RECETTE NAVIGATEUR N'A ÉTÉ JOUÉE** (le rôle `client` est exclusif, le propriétaire est connecté, un pilote lui prendrait sa place — **aucun humain n'a ouvert la page**), et **un onglet suiveur qui rouvre une fenêtre ne prévient PAS le porteur**, dont la liste dira « fermée » en permanence (limite de conception assumée : aucun ordre d'ouverture ne transite par le canal, `window.open` exigeant l'activation utilisateur de l'onglet qui a le geste). Plus de deux onglets n'est mesuré par rien.
- **lot 3 — la campagne sur la VM : LES DOUZE ITEMS JOUÉS (5 septembre 2026)** — [résultats partiels](docs/superpowers/plans/2026-09-05-lot3-campagne-vm-resultats-partiels.md) — la suspension du lot 3 est levée par un **harnais re-situé sur l'appliance**, posé à côté de `harnais.sh` qui reste intact. Chiffres datés : **latence** (item 7) — `captureTime` absent de 9 084/9 084 trames, substitut 9,18/9,04 ms au nominal contre 42,27/39,72 ms sous `netem adsl` ; **extinction** (item 5) — **3 sorties virtuelles orphelines** pour 3 fenêtres, deux exécutions, contre **0** au témoin à froid ; **file du capteur** (item 6) — `PROFONDEUR_MAX=64`, **0 refus pour 457 attachements** ; **replis micro** (item 9) — `Introuvable` couru **455** fois ; **applications** (item 11) — **41 au catalogue, 34 `non-mesuree`** ; **`Resize`** (item 12) — trois viewports servis exactement, **aucun maillon fautif**. 🔴 **LE CONSTAT LE PLUS TRANSFÉRABLE : TROIS CONTRÔLES QUE LE PLAN PRESCRIVAIT NE POUVAIENT PAS ÉCHOUER** — le `grep "NonMesuree"` (variant Rust jamais journalisé), le TÉMOIN `attache au capteur` (le produit écrit `attaché`, avec l'accent), et le commentaire de sonde dictant que le sender report RTCP alimente `captureTime` (il alimente `remoteTimestamp`). 🔵 **Et l'item 1 (3.7) — « temporaire + renommage », *le seul chemin par lequel une sauvegarde peut se perdre en silence* — est JOUÉ : aucune perte, aucun temporaire aux trois inventaires, et une ROUGE (`PONT_ECRITURE=0`) qui perd la sauvegarde pour de bon (44 o inchangé au poste local contre 148 o côté VM).** ⚠️ **CE DOCUMENT NE CLÔT PAS LE LOT POUR AUTANT** : l'item 4 rend **0/12 sessions**, bloqué par le mode `pomerium` (`/auth/connexion` → 404) et non par un défaut de pilote, et sa rouge est **vacueuse** ; plusieurs items concluent par un **blocage nommé** plutôt que par un chiffre, aucune ligne de produit n'a été modifiée, personne n'a regardé une image ni écouté un son.
- **sortie-designee-exemptee : la sortie DÉSIGNÉE cesse d'être refusée sur sa taille (1ᵉʳ septembre 2026)** — [résultats](docs/superpowers/plans/2026-09-01-sortie-designee-exemptee-resultats.md) — 🔴 **plus AUCUNE application ne pouvait s'afficher**, et ce n'était pas Steam : `creer_sortie` demandait une sortie virtuelle à la taille du viewport puis exigeait qu'elle naisse **au moins aussi grande**, or le pilote SudoVDA n'honore pas la taille demandée — **la même demande de 1614×1080 fait naître 1860×1080, 1614×1080 ou 780×492 selon l'instant** (troisième point de mesure pour l'inconnue de D8, toujours **incomprise**). Huit refus en boucle sur une sortie pourtant **DÉSIGNÉE par CCD**. Le lot 33 avait changé cette inconnue en panne totale en faisant suivre la demande au navigateur. Remède : sur une sortie **positivement désignée**, la taille devient une **CONTRAINTE et non un contrat** — `taille_pour_viewport` y ajuste déjà à rapport préservé ; `attachee_au_bureau`, `deja_prises` et la **nominativité** de l'exemption restent, et `None` rend le produit d'hier ligne pour ligne. 🔵 **Mesuré en production après déploiement : 16 fenêtres servies, 0 refus, et `exemptee=true` UNE seule fois** — sur la sortie née à 780×492, que le code d'hier refusait. 🔴 **Une affirmation du dépôt est devenue fausse et a été corrigée aux TROIS places** (« la désignation ne desserre aucun garde »). ⚠️ **CE QUE CE LOT N'ÉTABLIT PAS** : personne n'a regardé l'image, aucune recette navigateur n'a été jouée, **la boucle de replacement à 1 Hz (424 en 4 min) n'est ni expliquée ni corrigée**, et le `ensure_primary` d'Apollo — posé sur décision du propriétaire pour que Steam paraisse dans Moonlight — **n'a pas été éprouvé sous Moonlight**, alors que la conf portait déjà une réfutation mesurée de cette option dans un montage AVEC dummy plug (retiré depuis). 🔴 **Legs distinct : l'instance unique de Steam rend le legs d'appartenance du lot 32I SYSTÉMATIQUE — tout redémarrage de l'agent condamne Steam jusqu'à sa fermeture complète.** 🔵 **SUITE — le SURSIS** : le propriétaire a alors rapporté « plein d'onglets », et **la mesure a réfuté la correction évidente** — un inventaire en session 1 rend **26 fenêtres Steam dont UNE SEULE** satisfait déjà `merite_une_fenetre`, si bien que durcir ce critère aurait écarté du légitime sans toucher la cause. La cause est la **TRANSIENCE** : six des 23 fenêtres servies ont vécu **0,11 à 0,15 s**, et la pop-up survit à la fenêtre Windows. `superviseur::sursis` (module PUR, 6 tests) retient une fenêtre `DUREE_SURSIS` avant de l'annoncer, et `hook::merite_encore` **redemande à l'échéance** — sans cette seconde moitié le sursis ne serait qu'un retard. 🔵 **Deux chemins indépendants empêchent l'onglet** (le `Disparue` du hook, et `IsWindow` à l'échéance), donc le remède ne dépend pas de la fiabilité du hook. ⚠️ **`DUREE_SURSIS` = 500 ms N'EST PAS CALIBRÉE**, et **le sursis n'est PAS mesuré en production** : le contrôle qui vaudra est le compte d'onglets, pas la trace.
...
- ~~🔴 **ONZE PILOTES DE RECETTE VISENT UNE RACINE QUE CE CHANTIER A FERMÉE.** Le
  ...
  [Verdicts des douze](docs/superpowers/plans/journaux-lot3-pilotes/verdicts.md)
...
- 🔴 **UNE APPLICATION DU WINDOWS STORE NE SERAIT PAS ADOPTÉE** (lot 32I,
  ...
  [les résultats partiels du lot 3](docs/superpowers/plans/2026-09-05-lot3-campagne-vm-resultats-partiels.md).
...
- 🔴 **LA LATENCE DE BOUT EN BOUT — jamais, par aucun sous-bloc, depuis D1.**
  ⚠️ **TOUJOURS VRAI AU 5 SEPTEMBRE 2026, ET LA TENTATIVE EST INSTRUCTIVE.**
  La méthode que le plan du lot 3 prescrivait — `presentationTime -
  captureTime` par `requestVideoFrameCallback` — **ne peut rien rendre** :
  `metadata.captureTime` est absent de **9 084 trames sur 9 084**. La cause
  est tranchée et n'est PAS que l'agent se taise (204 sender reports RTCP sur
  le fil, 63 lus par Chrome) : `captureTime` vient de l'extension d'en-tête RTP
  **`abs-capture-time`**, que l'agent **ne négocie pas** (`grep` : 0).
  🔵 **Un SUBSTITUT est mesuré** — `RTT/2 + totalProcessingDelay` — **9,18 et
  9,04 ms** au nominal contre **42,27 et 39,72 ms** sous `netem adsl` ; il
  **borne par le bas** et ne contient ni la capture, ni l'encodage, ni
  l'affichage après `presentationTime`. [Verdict de l'item 7](docs/superpowers/plans/journaux-lot3-latence/verdict.md).
- 🔴 **AUCUNE CONSTANTE N'EST CALIBRÉE, ET AUCUN JUGEMENT D'USAGE N'A ÉTÉ
  PORTÉ** — ni visuel, ni d'écoute. 🔵 **ENTAMÉ, ET SEULEMENT ENTAMÉ, LE
  31 AOÛT 2026** : le lot 33 a **lu les pixels** d'une fenêtre servie et versé
  sonde, relevé, témoin et images au dépôt
  (`docs/superpowers/plans/journaux-lot33/pixels/`). ⚠️ **Ce n'est pas un
  jugement d'USAGE** — c'est une mesure de couleur sur quatre bords, qui ne dit
  rien de la latence, de la netteté ni du confort. Le manque reste ouvert ; ce
  qui a changé, c'est qu'un instrument existe désormais pour le combler. La liste est longue et ouverte : `BPP_MIN`
...
| --- | --- |
| **D** multi-fenêtres | l'A/B sur `set_desired_bitrate` (**écarté par décision**, condition de réouverture : charge d'hôte **contrôlée**, ≥ 8 paires) ; ~~le maillon fautif du `Resize` **non identifié**~~ **AUCUN MAILLON FAUTIF SUR LE CHEMIN MESURÉ LE 5 SEPTEMBRE 2026** : trois viewports (1024×700, 1400×500, 640×900 — rapports 1,46 / 2,80 / 0,71) traversent les cinq maillons **intacts**, jusqu'à `videoWidth×videoHeight` relevés dans la page. ⚠️ **La rouge prescrite — un `Resize` en mode `SortieEntiere` — n'a PAS été jouée**, et personne n'a regardé l'image. [Verdict de l'item 12](docs/superpowers/plans/journaux-lot3-resize/verdict.md) ; **six constats de revue PERDUS** avec un rapport gitignoré ; ~~aucune **cause naturelle** de mort de capture audio~~ **DEUX CAUSES TENTÉES, AUCUNE NE TUE — MESURÉ LE 5 SEPTEMBRE 2026** : désactiver puis réactiver le point de terminaison de rendu (`Speakers (Steam Streaming Speakers)`, état final `OK`) et arrêter puis relancer `Audiosrv` (`Running → Stopped → Running`) laissent la capture **vivante et lisante** (`compteurs audio … actif=true rejetes=0`, relevé APRÈS la cause). 🔵 **Le témoin négatif est ce qui donne son sens à ce zéro** : `AUDIO_FAUTE_LECTURE=15` provoque une mort que l'instrument VOIT — **15** fautes injectées, **14** re-tentatives, **1** `capture arrêtée définitivement`, **1** `capture audio reconstruite`, comptes qui tombent sur `LECTURES_ECHOUEES_MAX = 10` et `RECONSTRUCTIONS_MAX = 3`. ⚠️ **UNE TROISIÈME CAUSE N'A PAS PU ÊTRE TENTÉE** — basculer le périphérique de rendu par DÉFAUT — faute du module `AudioDeviceCmdlets` sur l'invité (**0 module**) : c'est la dette **C7**, pas le produit. ⚠️ **Un échec à trouver n'est PAS une preuve d'absence**, la liste des causes n'est pas exhaustive, et le témoin ne sait provoquer qu'une mort par ÉCHEC DE LECTURE. [Verdict de l'item 10](docs/superpowers/plans/journaux-lot3-audio/causes-tentees.md) |
| **E** microphone | 🔴 **personne n'a écouté** — le critère de fin n'est atteint que par un juge logiciel ; le son d'une **autre application** n'est pas annulé (−13 dB, donc **amplifié**) et rien ne le dit à l'utilisateur ; le rééchantillonnage 48 → 44,1 kHz du câble, **remède inapplicable** ; ~~deux replis livrés et **jamais courus**~~ **VRAI D'UN SEUL, MESURÉ LE 5 SEPTEMBRE 2026** : le repli `Introuvable` court **455 fois**, une par session — VB-Audio n'est pas installé sur l'appliance, donc la désignation intégrée ne correspond à rien — et il se comporte comme la spec l'exige (aucun fil de rendu, l'inventaire dans le `warn!`, **aucun repli vers le défaut de Windows** alors qu'un endpoint est disponible). Le repli `Ambigu`, lui, est **INEXERÇABLE sur cette VM** : un seul point de terminaison de rendu ACTIF existe, donc aucune sous-chaîne ne peut être ambiguë. [Verdict de l'item 9](docs/superpowers/plans/journaux-lot3-micro/verdict.md) ; la licence VB-Audio est **personnelle seulement** |
| **③** pont fichiers | 🔴 **débit et mur de lecture CONFIRMÉS le 5 septembre 2026, deux exécutions** : **32,82 Kio/s** sur un palier de 78 s (20 fichiers DISTINCTS — relire le même rendait 1 312 669 Kio/s, c'est-à-dire le cache de Windows), et **128 Kio lisible / 192 Kio ÉCHOUE** aux deux exécutions ; ~~🔴 **aucun listage de plus de ~3 150 entrées n'aboutit** (taille d'un message SCTP)~~ 🔴 **LE MUR D'ÉNUMÉRATION A BOUGÉ** : **3 200 entrées ABOUTIT, COMPLET**, aux deux exécutions — F4 le donnait pour échouant — et **4 000 échoue** ; le mur est donc entre 3 200 et 4 000, **et la cause de l'écart n'est PAS identifiée**. 🔴 **DÉFAUT NEUF, mesuré et NON corrigé : après un échec de lecture, la résolution du répertoire SUIVANT échoue en « does not exist » sur un répertoire qui EXISTE** — l'anomalie suit la POSITION et non le rang (isolée en sondant un rang témoin AVANT toute lecture) ; un cache de chemin négatif ProjFS est le suspect, rien ne l'établit. [Verdict de l'item 2](docs/superpowers/plans/journaux-lot3-pont-murs/verdict.md) ; ~~🔴 **l'idiome « temporaire + renommage » n'a jamais été exercé sur un éditeur réel** — *le seul chemin par lequel une sauvegarde peut se perdre en silence*~~ **EXERCÉ LE 5 SEPTEMBRE 2026, ET AUCUNE SAUVEGARDE NE SE PERD** : le Bloc-notes écrit **EN PLACE** dans la racine ProjFS, et l'enregistrement arrive **entier** au poste local en ~56 ms — deux exécutions (44 o → 70 o puis 44 o → 96 o, sha256 changés, **1** acquittement chacune), **aucun temporaire aux TROIS inventaires** (dont un pris l'éditeur ENCORE OUVERT, 3 s après `Ctrl+S`), et les NOMS relevés plutôt qu'un compte — *un temporaire créé puis renommé laisserait le compte inchangé*. 🔵 **La ROUGE (`PONT_ECRITURE=0`) PERD LA SAUVEGARDE POUR DE BON** : poste local **44 o INCHANGÉ** pendant que l'invité croit avoir enregistré (148 o, marqueur présent), **0** acquittement. ⚠️ **La rouge du plan (`PONT_MUTATION=0`) aurait été VERTE** — elle désarme un renommage qui n'a pas lieu. ⚠️ **UN SEUL ÉDITEUR EXISTE SUR CETTE APPLIANCE** : `notepad.exe` **True**, `wordpad.exe` **False**, VS Code **False** — or VS Code (sauvegarde atomique), Word et LibreOffice sont **précisément** les outils qui emploieraient l'idiome : **le cas où la perte serait possible reste NON ÉPROUVÉ**. [Verdict de l'item 1](docs/superpowers/plans/journaux-lot3-pont-sauvegarde/verdict.md) ; ~~un renommage fait **disparaître un répertoire frère**~~ **NE SE REPRODUIT PLUS, MESURÉ LE 5 SEPTEMBRE 2026** : trois bras (cache armé ×2, `PONT_CACHE=0`), `D sous-dossier` présent dans **5 listages sur 5** à chacun — dont un second sans `Rafraichir` et un troisième après plus de `TTL_ENUMERATION`. 🔴 **Le témoin négatif est ce qui rend ce verdict lisible** : la suppression réelle de `temoin-2.txt` **disparaît** du listage aux trois bras, donc le listage sait perdre une entrée. Le cache, que F5 accusait de *prolonger* le défaut, ne change plus rien — il n'y a plus de défaut à prolonger. ⚠️ **Une non-reproduction n'est PAS une preuve d'absence** : aucun commit n'est nommé, le geste est un renommage de FICHIER (pas de répertoire), un seul jeu. [Verdict de l'item 3](docs/superpowers/plans/journaux-lot3-pont-frere/maillons-verdict.md) ; aucune éviction, le disque grossit |
| **④** gestion d'apps | ~~🔴 **un `<img src>` ne porte pas d'`Authorization`**~~ **FERMÉ (lot 16, 30 août 2026)** : sur décision du propriétaire du dépôt, `GET /application/:id/icone` s'atteint désormais par une **URL SIGNÉE** — sous-clé HKDF dérivée du secret de jeton, signature couvrant l'application, la VM et l'expiration, durée de 5 à 6 minutes, frappée par le seul `GET /applications` (donc sous jeton porteur, après le contrôle d'appartenance de la VM), et **l'ancien chemin à `Authorization` est RETIRÉ** — `plateforme/src/apps/url-icone.ts`. 🔴 **CE QUI RESTE OUVERT, ET CE N'EST PAS LA MÊME CHOSE : le hub n'est toujours PAS ÉTABLI installable**, pour deux raisons distinctes — ① aucun navigateur ne l'a jugé (il faut un humain, ou un Chromium piloté) ; ② `/hub.webmanifest` ne déclare **aucun** `icons`, et **une URL signée ne peut pas la lui donner** : ce manifeste est statique, engendré au build, et une URL qui expire en cinq minutes n'y a pas sa place — il appelle une icône **statique**, donc un choix d'image qui appartient au propriétaire. ⚠️ Et une URL signée placée dans un `icons[].src` **ne serait pas atteignable derrière Pomerium** : un manifeste est allé chercher SANS cookie, et cette URL ne finit ni par `.png`, ni par `.ico`, ni par `manifest.json` — les trois seuls suffixes que la politique laisse passer. Ses `file_handlers` restent donc inertes ; ~~71 applications restent `NonMesuree`~~ **FAUX, MESURÉ LE 5 SEPTEMBRE 2026 : le catalogue servi par la plateforme compte 41 entrées AU TOTAL, dont 34 `non-mesuree`, 6 `pixels:256` et 1 `pixels:48`.** 🔴 **Et le contrôle qui produisait « 71 » ne peut pas exister : `NonMesuree` est un variant d'énumération Rust, JAMAIS une chaîne journalisée (0 occurrence sur 169 993 lignes) — sur le fil la sérialisation est `"source_max":"non-mesuree"`.** Voir [le verdict de l'item 11](docs/superpowers/plans/journaux-lot3-apps/verdict.md) ; une icône qui change **sans que le raccourci change** n'est jamais revue — **toujours non mesuré** ; ~~le magasin n'est **jamais nettoyé**~~ **FERMÉ (lot `legs-sans-vm`)** : `apps/nettoyage.ts` câble une éviction par âge sur les deux magasins (icônes, tranches), avec un plancher de référence contre la corruption, câblée sur la base réelle au démarrage du service — voir le document de résultats du lot pour les legs qu'elle laisse (course stat→rm, fenêtre d'obsolescence de l'instantané d'icônes) |
...
| **⑥** design system | le **WCO** n'a jamais été rendu — **écarté du lot `legs-sans-vm` par décision : inatteignable tant que le hub n'est pas installable**, une décision de sécurité qui appartient au propriétaire ; 🔴 **`--accent-fenetre` EST PEINTE PAR RIEN — LEGS ROUVERT LE 31 AOÛT 2026** (lot 33). ~~**FERMÉ (lot `legs-sans-vm`)** : déclaré dans `tokens/couleurs.css`, peint sur `#remote` par `style.css`~~ — **la peinture a été RETIRÉE sur décision du propriétaire du dépôt**, qui a choisi en connaissance de cause : la bordure d'1 px de `#remote` était la seule marge restant autour de l'image une fois le recadrage et le cadre visible mis en coïncidence. **Ce qui subsiste est un mécanisme complet qui n'aboutit à rien de visible** : le capteur LIT la couleur (Win32), l'agent l'ANNONCE, `accent-dom.ts` la POSE en token — et **aucune feuille ne la lit**. ⚠️ **RÉGRESSION ASSUMÉE ET NOMMÉE, jamais un oubli** ; le token, `accent-dom.ts` et le chemin agent sont **conservés à dessein**, pour que repeindre cette couleur ailleurs reste bon marché. 🔴 **ET LE GARDE DES ORPHELINS (§7.6) NE PEUT PAS LE VOIR** : il compte `poserToken(...)` — un **ÉCRIVAIN** — comme un appelant (son propre commentaire dit que ce bras a été ajouté pour cela), si bien qu'il rend `0 orphelin(s)` alors que le token n'a plus **aucun LECTEUR**. **Le contrôle n'a pas été assoupli : il est structurellement aveugle à ce cas**, et c'est dit ici plutôt que masqué par son vert ; 🔴 **ET CE CAS A UN JUMEAU DEPUIS LE 31 AOÛT 2026 — LE PIÈGE EST DONC PAYÉ DEUX FOIS, ET IL EST REMONTÉ AUX PIÈGES TRANSVERSES** : `.bureau` (`client/src/bureau/bureau.css`) n'est employée par **aucun balisage** depuis que le hub porte son propre conteneur, et elle est **conservée artificiellement** parce que sa déclaration `margin-block: var(--e-7)` est le **SEUL emploi de `--e-7` dans tout le dépôt** — la retirer ferait échouer §7.6. **Du code mort gardé pour satisfaire un contrôle**, honnêtement déclaré dans l'en-tête du fichier ; chantier `navigation-hub-unique` ; `client/src/design/tokens.css` **n'est plus à sa porte** — scindé en `tokens/couleurs.css` et `tokens/echelles.css` le 25 août 2026, chacun avec sa propre porte armée et son découpage suivant nommé (voir leur en-tête). 🔴 **AUCUNE TAILLE N'EST RECOPIÉE ICI, ET C'EST DÉLIBÉRÉ** : la première rédaction de cette ligne portait trois nombres, et le round qui les a écrits les a rendus faux **dans le même round** — 45/191/149 annoncés, 64/201/167 mesurés. `wc -l` sur les trois fichiers — avec **treize** lecteurs qui le nommaient par son chemin AVANT ce découpage (« onze » et « 33 » étaient tous deux vrais de choses différentes, faute de règle énoncée ; chaque lecteur explique désormais son choix dans son propre en-tête) ; ~~les galeries n'ont **aucun test**~~ **FERMÉ (lot `legs-sans-vm`)** : `galerie.test.ts` et `galerie-primitives.test.ts` figent la LISTE des cas (tokens, primitives balisées) — **aucun jugement visuel**, ce que ces deux fichiers disent eux-mêmes ne pas établir |
| **①** divers | ~~le **propriétaire mono-fenêtre n'existe pas** (ni presse-papier, ni accent)~~ **MESURÉ LE 5 SEPTEMBRE 2026, ET LE VERDICT EST *COHÉRENT AVEC* CE LEGS SANS LE DÉMONTRER SEUL** : sur le même geste — trois écritures du presse-papier en session 1, chacune relue identique — le produit livré pousse **3 messages et 1 accent**, le mode `SUPERVISEUR=0 CAPTEUR=0` en pousse **0 et 0**, et la rouge `PRESSE_PAPIER=0 ACCENT=0` en pousse **0 et 0** avec ses **deux traces de désarmement** et **1 fenêtre servie**. 🔴 **CE QUE LA MESURE NE SÉPARE PAS** : en mono-fenêtre l'agent meurt à l'activation de l'encodeur (`Catastrophic failure 0x8000FFFF`, défaut du lot 31), donc « aucun propriétaire » et « aucune session » rendent LE MÊME ZÉRO — la lecture de code les sépare (les deux propriétaires vivent dans le **capteur**, absent quand `CAPTEUR=0`), la mesure seule non. [Verdict de l'item 8](docs/superpowers/plans/journaux-lot3-mono/verdict.md) ; ~~le canal `Message` du registre est **non borné**, et trois sous-blocs l'ont aggravé~~ **FERMÉ (lot `legs-sans-vm`)** : une file bornée par coalescence par variante (`capteur/sommeil/file.rs`), branchée sur les cinq points d'appel de production, avec trace au palier ; ce lot n'a rien exercé en charge réelle — voir le document de résultats ; le **niveau 2** du presse-papier (qu'un humain puisse coller) reste **non mesurable** |
