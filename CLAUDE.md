...
- **lot 33 — le recadrage et la fenêtre suivent le viewport, à la forme demandée (31 août 2026)** — [résultats](docs/superpowers/plans/2026-08-31-lot33-viewport-suivi-resultats.md) — 🔴 **le `Resize` du navigateur était JETÉ, et c'est mesuré sur l'agent de production** : 34 demandes, rapports d'aspect de **1,105 à 3,559**, toutes servies à un rapport fixe — le `#remote` étant `object-fit: contain` sur `--video-letterbox: #000`, l'écart **se peint littéralement en noir**. 🔴 **Les deux moitiés du défaut se couvraient l'une l'autre** : `viewport_recu` ignorait tout viewport sur une session `Vivante`, et le client n'en réannonçait aucun. Remède : **une règle pure (`taille_pour_viewport`), deux processus, UNE seule mesure côté client** — le capteur retaille fenêtre + recadrage + encodeur **sans relâcher la duplication** (C1 de D1), le superviseur corrige `taille_sortie` et repose ; même `HMONITOR`, même borne, donc le second geste est un `no-op`. 🔵 **La borne est la ZONE DE TRAVAIL** — premier lecteur de `rcWork` du dépôt — ce qui sort les 48 rangées de la barre des tâches du cadre et rend à l'application ses 48 px. 🔴 **DEUX DÉFAUTS DE MOI, TROUVÉS APRÈS LE PREMIER DÉPLOIEMENT ET CORRIGÉS PAR UN SECOND** : ① la composition `borner_a_la_taille_max` (préserve l'aspect) ∘ `taille_retenue` (`min` axe par axe) **détruisait le rapport À L'INTÉRIEUR de la borne** — écarts de **2,4 % à 4,7 %** mesurés par capture réseau sur les 28 trames `viewport` de ses gestes réels, remplacée par un **fit à aspect préservé** ; ② toutes mes traces étaient posées **après un court-circuit**, si bien qu'un `0` au journal ne distinguait plus un défaut de la limite déclarée — **je m'étais rendu aveugle à mon propre échec**, en retirant précisément la trace inconditionnelle qui avait rendu le diagnostic possible. ⚠️ **CE QUE CE LOT N'ÉTABLIT PAS : personne n'a regardé une image**, et la marge sur les **quatre** côtés reste **inexpliquée** — `object-fit: contain` ne peut produire qu'une paire de bandes, et deux candidats sont nommés sans qu'aucun soit établi.
